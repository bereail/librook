...
export default function BookSearchModal({ onSelect, onClose }) {
  ...
  const handleChange = (e) => {
    ...
    setQuery(val)
    search(val, soloEspanol ? { lang: 'spa' } : {})
  }

  const handleSelect = (book, tipo) => {
    onSelect({ ...book, tipo }) // tipo: 'leido' | 'pendiente'
  }
    ...
    <div className={styles.overlay} ref={overlayRef} onClick={e => e.target === overlayRef.current && onClose()}>
      <div className={styles.modal}>
        <div className={styles.header}>
          <div>
            <h2 className={styles.title}>Buscar en Open Library</h2>
        ...
        <div className={styles.searchWrap}>
          ...
          {query && !loading && (
            <button className={styles.clearBtn} onClick={() => setQuery('')} aria-label="Limpiar">
        ...
        <div className={styles.langRow}>
          <label className={styles.langToggle}>
            <div
              ...
              tabIndex={0}
              onKeyDown={e => e.key === ' ' && setSoloEspanol(v => !v)}
            >
